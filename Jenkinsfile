pipeline {
  ...
  environment {
    ...
    EXT_REPO = 'docker-ffmpeg'
    BUILD_VERSION_ARG = 'FFMPEGWEB_COMMIT'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-ffmpeg'
    CONTAINER_NAME = 'ffmpeg'
    DOCKERHUB_IMAGE = 'lsiodev/ffmpeg-web'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/ffmpeg-web'
    PR_DOCKERHUB_IMAGE = 'lspipepr/ffmpeg-web'
    DIST_IMAGE = 'ubuntu'
    MULTIARCH='false'
    CI='true'
    CI_WEB='true'
  ...
  stages {
    ...
    // If this is a web build use live docker endpoints
    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=web -v ${TEMPDIR}:/ansible/jenkins linuxserver/jenkins-builder:latest 
              docker pull linuxserver/doc-builder:latest
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=web -v ${TEMPDIR}:/ansible/readme linuxserver/doc-builder:latest
              if [ "$(md5sum ${TEMPDIR}/${LS_REPO}/Jenkinsfile | awk '{ print $1 }')" != "$(md5sum Jenkinsfile | awk '{ print $1 }')" ] || \
                ...
                git --git-dir ${TEMPDIR}/repo/${LS_REPO}/.git checkout -f web
    ...
    stage('Github-Tag-Push-Release') {
      ...
      steps {
        ...
        -d '{"tag":"'${EXT_RELEASE_CLEAN}'-ls'${LS_TAG_NUMBER}'",\
             "object": "'${COMMIT_SHA}'",\
             "message": "Tagging Release '${EXT_RELEASE_CLEAN}'-ls'${LS_TAG_NUMBER}' to web",\
             "type": "commit",\
